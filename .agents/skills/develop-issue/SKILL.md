...
3. Before review, apply the permission-boundary checkpoint and explicit human takeover in “P1.1 — supervised workflow pilot acceptance” in the process. Do not infer a technically read-only child from a workspace-write parent or configured TOML values; unresolved boundary evidence pauses review as BLOCKED. The user may explicitly provide a supported read-only review session in the same client, with a fresh context and the existing QA role document. Record that human takeover explicitly; it is not an automatic native child handoff. If the appropriate environment or required evidence is unavailable, remain BLOCKED. Otherwise use fresh native `pilot_qa` when its boundary is established, or the explicitly provided review session, with the exact specification, candidate/base, checkpoint, and independently inspectable artifacts/CI evidence. Apply the process's evidence-based PASS/FAIL/BLOCKED rules; SWE claims cannot establish PASS and unavailable evidence must remain NOT VERIFIED.
...
PM and QA are required to remain read-only and make no remote writes; configured settings alone do not establish effective permissions. Apply the process's environment checkpoint before live role work, including PM. The explicit user-provided QA review route above is the only review fallback; never bypass configuration, credentials, or managed policy. Preserve every approval and permission boundary in the process. This skill grants no cloud actions, deployments, repository-permission changes, or automatic next-Issue work.
